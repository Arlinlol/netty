...
public final class ChannelOutboundBuffer {
    ...
    private static void safeSuccess(ChannelPromise promise) {
        if (!(promise instanceof VoidChannelPromise) && !promise.trySuccess()) {
            ...
            if (err == null) {
                logger.warn("Failed to mark a promise as success because it is succeed already: {}", promise);
            } else {
                logger.warn("Failed to mark a promise as success because it is failed already: {} with cause {}",
    ...
    private static void safeFail(ChannelPromise promise, Throwable cause) {
        if (!(promise instanceof VoidChannelPromise) && !promise.tryFailure(cause)) {
            Throwable err = promise.cause();
            if (err == null) {
                logger.warn("Failed to mark a promise as failure because it's succeed already: {}", promise, cause);
            } else {
                logger.warn("Failed to mark a promise as failure because it's failed already: {} with cause {}",
